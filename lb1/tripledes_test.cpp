...
using namespace TripleDES_Implementation;
...
void test_triple_des_mode(const std::string& file, CipherMode mode, PaddingScheme padding, ExtraParams params = {}) {
    std::cout << "\n[TripleDES] Testing file: " << file << " | mode: ";
    switch (mode) {
    ...
    byte_array iv  = { 0x12, 0x34, 0x56, 0x78, 0x90, 0xAB, 0xCD, 0xEF };
    ...
    auto triple_des_algo = std::make_unique<TripleDES>();
    CipherContext ctx(std::move(triple_des_algo), key, mode, padding, iv, params);
...
int main() {
    try {
        ...
        for (const auto& file : files) {
            ...
            for (auto mode : modes) {
                try {
                    test_triple_des_mode(file, mode, padding);
                }
            ...
            test_triple_des_mode(file, CipherMode::RANDOM_DELTA, padding, delta_params);
        }
    }
    catch (const std::exception& ex) {
